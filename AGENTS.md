...
- Releases: see [dev-docs/releases-and-packaging.md](dev-docs/releases-and-packaging.md) for GoReleaser, Homebrew (`brews` → `Formula/llml.rb`), optional Actions secrets, and maintainer automation. Summary: push a `v*` tag after updating repo-root `VERSION` to match; `.github/workflows/release.yml` publishes GitHub Release archives. Users install via `brew tap flyingnobita/llml https://github.com/flyingnobita/llml.git` then `brew install llml` (a bare tap name resolves to the wrong GitHub repo).
