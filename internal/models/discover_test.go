...
func TestSearchRoots_dedupes(t *testing.T) {
	t.Parallel()
	home, err := os.UserHomeDir()
	if err != nil {
		t.Fatal(err)
	}
	s := settings.Settings{ExtraModelPaths: []string{"/foo/bar", "/foo/bar", "/baz"}}
	got := s.SearchRoots([]string{filepath.Join(home, "models")}, false)
